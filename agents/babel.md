...
4. **`user_stack_hints` — capture-verbatim-or-empty rule.** If the user explicitly names a programming language (Python, Go, Kotlin), framework (FastAPI, Spring Boot, Flutter), database (SQLite, PostgreSQL, MongoDB), cloud (AWS Lambda, Cloudflare Workers), or notable library, list it verbatim. If the user names nothing, leave the array empty. **NEVER invent or infer stack** — only capture what is literally there. This list is downstream-load-bearing: Architect treats it as a hard binding that overrides tier-default stacks (e.g. tier=T2 BaaS would normally imply Supabase, but if the user said "SQLite", the user wins).
