...
def check_audit_log_dir(repo_root: Path) -> DoctorCheck:
    audit_path = Path(
        os.getenv("AUDIT_LOG_PATH", str(repo_root / "runtime" / "audit" / "decisions.jsonl"))
    )
...
def check_tier_templates() -> DoctorCheck:
    ...
    try:
        from runtime.architecture.bootstrap import _FRAMEWORK_PACKAGES
        ...
        return DoctorCheck(
            ...
            CAT_RECOMMENDED,
            fix_hint="restore runtime/architecture/bootstrap.py",
        )
