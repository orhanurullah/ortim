...
### Added
- **Browser device-code login (`ortim cloud login`, no arguments).**
  Most platform accounts are Google sign-in and have no password — the
  email+password login locked them out of the CLI. The default login is
  now a simplified RFC 8628 flow: the CLI shows a short code, opens
  `cloud.ortim.dev/device`, and polls until the signed-in user approves.
  The stored config now also keeps the refresh token. The legacy path
  survives as `ortim cloud login <email> [--password …]`.
- **SECURITY.md** — vulnerability reporting, short threat model, and a
  channel-by-channel table of what leaves your machine (and what never
