...
| **2.3-pp** | Faz 2.3 live mutation-testing run — measure DeepSeek Reviewer catch rate on the 6 shipped cases | Run `ortim mutation-test --live --provider=deepseek`. Expected spend ~\$0.02. Target ≥%70 strict catch; <%50 triggers Reviewer prompt sertleştirme. Defer to a separate session so the result lands as a clean data point. | — |
...
- **DEFERRED:** 5 (added 2026-05-16: 2.2-pp Faz 2.2 live proof-point — awaiting local Ollama install; 2.3-pp Faz 2.3 live mutation-test run — DeepSeek, separate session)
