...
# ---- T0/web templates (single-file CLI) ------------------------------

_T0_WEB_GITIGNORE_EXTRA_PYTHON = """\
__pycache__/
*.py[cod]
.pytest_cache/
.venv/
...
def _t0_web_python_pyproject(project_name: str) -> str:
    """Minimal pyproject.toml — declares project metadata + pytest dev
    dep so the test runner has something to invoke. Worker fills in
    real dependencies as the brief demands; we keep the starting set
    empty to avoid prescribing a stack the brief doesn't justify."""
    safe = (
        project_name.strip().replace(" ", "-").lower() or "ortim-cli"
    )
    return (
        f'[project]\n'
        f'name = "{safe}"\n'
        f'version = "0.1.0"\n'
        f'description = "Generated by Ortim T0/web bootstrap"\n'
...
def _t0_web_go_mod(project_name: str) -> str:
    """Minimal go.mod — sufficient for `go build`, `go test ./...` to
    resolve. The example.com/ host is conventional for unpublished
    modules; the operator can rename without breaking anything."""
    safe = (
        project_name.strip().replace(" ", "-").lower() or "ortim-cli"
    )
    return f"module example.com/{safe}\n\ngo 1.22\n"
...
def _resolve_t0_language(
    ...
    None when the signal is too weak to commit to a template."""
    if locked_stack is not None and getattr(locked_stack, "language", None):
        raw = locked_stack.language.strip().lower()
        if "python" in raw:
            return "python"
        if raw.startswith("go") or "golang" in raw:
            return "go"
        if "bash" in raw or "shell" in raw:
            return "bash"
    ...
    try:
        ...
        return None
    # Word-anchored hints — same vocabulary as `_LANG_TEST_CMD`.
    if "python" in text or "fastapi" in text or "pytest" in text:
        return "python"
    if "golang" in text or "cobra" in text or "go " in text:
        return "go"
    if "bash" in text:
        return "bash"
    return None
...
def _framework_to_packages(primary_framework: str | None) -> list[str]:
    ...
    print(
        ...
        f"_FRAMEWORK_PACKAGES in ortim/architecture/bootstrap.py.",
        file=sys.stderr,
    )
    return []
        ...
        for lib in all_libs:
            ...
            if entry is None:
                ...
                print(
                    ...
                    f"ortim/architecture/bootstrap.py.",
                    file=sys.stderr,
                )
                continue
            kind, version = entry
...
def _token_matches(token: str, text: str) -> bool:
    """Word-boundary-safe token check.

    Plain substring matching was the original (item 18a) but trips on
    ...
    """
    if token.endswith(" "):
        # Trailing space is an explicit word-boundary hint from the
        # original `_LANG_TEST_CMD` design — keep that contract intact.
...
def _infer_test_cmd_from_rfc(workspace: Path) -> str | None:
    ...
    Architect §4 surfaces explicit language and framework names, so
    word-boundary token checks are reliable enough for the greenfield
    path. Brownfield projects use the codebase reader's framework
...
def bootstrap_workspace_layout(
    ...
    # T0/web is a single-file CLI tier — no node_modules, but language-
    # specific ignores (Python __pycache__, Go bin/) matter. Resolved
    # once and reused for both the gitignore branch and the root-file
    # branch so we don't pick a different language for the two.
    t0_web_language = (
        _resolve_t0_language(locked_stack, workspace)
        if (tier == "T0" and app_class == "web")
        else None
    )
    ...
    if not gitignore.exists():
        ...
        if t0_web_language == "python":
            body += _T0_WEB_GITIGNORE_EXTRA_PYTHON
        elif t0_web_language == "go":
            body += _T0_WEB_GITIGNORE_EXTRA_GO
    ...
    # T0/web — single-file CLI tier. The web template above writes
    # package.json + tsconfig.json which assume a TypeScript stack;
    # T0/web's canonical picks are Python or Go (per
    # `_LANG_STACK_BY_TIER_APP`), so we emit language-specific minimal
    # scaffolding instead. Bash / unresolved language falls through to
    # the universal layout — no root files beyond .gitignore.
    if t0_web_language == "python":
        pyproject = workspace / "pyproject.toml"
        if not pyproject.exists():
            pyproject.write_text(
                _t0_web_python_pyproject(project_name), encoding="utf-8"
            )
            created.append(pyproject)
    elif t0_web_language == "go":
        gomod = workspace / "go.mod"
        if not gomod.exists():
            gomod.write_text(
                _t0_web_go_mod(project_name), encoding="utf-8"
            )
